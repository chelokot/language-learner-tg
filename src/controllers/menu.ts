...
import { kbExercisesForVocab, kbMenu, kbVocabularies, kbVocabulary } from '../ui/keyboards.js';
...
async function showVocabularies(ctx: CustomContext) {
  ...
  const currentLabel = current
    ? `${current.name} (${(current.goal_code || current.goal_language).toUpperCase()}→${(current.native_code || current.native_language).toUpperCase()})`
    : 'none';
  await ctx.reply(`Vocabularies${vocabs.length ? `nCurrent: ${currentLabel}` : ''}`, {
...
async function showVocabulary(ctx: CustomContext, id: number) {
  ...
  if (!vocab) return;
  const pair = `${vocab.goal_language} → ${vocab.native_language}`;
...
async function showExercises(ctx: CustomContext) {
  // Require at least one vocabulary
  const vocabs = await listVocabularies({
    db: ctx.db,
    ownerId: ctx.dbEntities.user.user_id,
  });
  ...
  // Require selected vocab (auto-select first if missing)
  ...
  await ctx.reply(
    `Your current selected vocabulary is “${current.name}” (${current.goal_language} words). Go to /vocabs to change it.nnChoose translation exercise to train.n“Word” = one word prompt. “Sentence” = sentence prompt.nFor ${current.goal_language}→${current.native_language}, the prompt will include at least one word from your list.nFor ${current.native_language}→${current.goal_language}, your answer must include at least one word from your list.`,
...
// Exercise selectors
menuController.callbackQuery(/exercise:(word|sentence):(gn|ng)/, ctx =>
  ctx.conversation.enter(CONVERSATION_NAMES.exercise, ctx.match![1], ctx.match![2]),
);
...
export async function createVocabularyConversation(
  ...
  await ctx.reply('Enter the name of the language you are learning (Goal language):');
  const goalLanguage = await waitText(conversation);
  await ctx.reply('Enter the name of your language used for translations (Native language):');
  const nativeLanguage = await waitText(conversation);
  await ctx.reply('Now enter a name for this vocabulary:');
  ...
  // Auto-select if user had none selected
  ...
  await ctx.reply(
    `Created vocabulary “${vocab.name}” (${vocab.goal_language} → ${vocab.native_language}). It is now selected.`,
  );
  await showVocabularies(ctx);
}
...
export async function addWordConversation(
  ...
  while (true) {
    ...
    await ctx.reply(`Type its translation in ${n}, or tap /auto to translate automatically.`);
...
Now enter the next ${g} word, or /stop to finish.`);
        ...
        if (maybeFix === '/fix' && lastWordId) {
          ...
          if (back !== '/stop') {
            // cheap update by delete+reinsert for simplicity (or implement editWord)
            await ctx.db.query('UPDATE word SET back=$2 WHERE id=$1', [lastWordId, back]);
          ...
          // it's the next word; push it back into flow
          if (maybeFix) {
            // emulate user typed next word: reuse loop, but we need to process it as "front"
            // easiest is to set front to that and continue
            // However we already added previous word; so continue by setting a temp variable
            // To keep flow simple here, just queue it by issuing a synthetic prompt
            await ctx.reply(`Type a word in ${g}. Send /stop to finish.`);
            // inject the previously read text back to conversation is complex; so we let user type again
          }
        }
        continue;
      } else {
    ...
    lastWordId = saved.id;
    await ctx.reply(`Saved “${front}” → “${back}”. Enter next ${g} word, or /stop to finish.`);
  }
  await showMenu(ctx);
}
...
export async function exerciseConversation(
  ...
  while (true) {
    ...
    if (kind === 'word') {
      if (dir === 'gn') {
        await ctx.reply(`Translate this word from ${g} to ${n}:n${word.front}`);
        ...
        await ctx.reply(`Translate this word from ${n} to ${g}:n${word.back}`);
        const answer = await waitText(conversation);
        if (answer === '/stop') {
          await ctx.reply('Stopping exercise');
          break;
        ...
        await ctx.reply(ok ? 'Correct' : `Incorrect. Right answer: ${word.front}`);
      }
    } else {
      // sentence
      if (dir === 'gn') {
        const sentence = await generateSentenceWithTerm(g, word.front, 'goal');
        await ctx.reply(`Translate this sentence from ${g} to ${n}:n${sentence}`);
        const answer = await waitText(conversation);
        if (answer === '/stop') {
          await ctx.reply('Stopping exercise');
          break;
        ...
        await ctx.reply(
          `Translate this sentence from ${n} to ${g}:n${sentence}nRemember to include at least one word from your list.`,
        );
        const answer = await waitText(conversation);
        if (answer === '/stop') {
          await ctx.reply('Stopping exercise');
        ...
        const result = await judgeTranslation(sentence, answer, n, g, word.front);
        await ctx.reply(result.ok ? `Correct. ${result.feedback}` : `Not quite. ${result.feedback}`);
      }
    }
  }
